    ...
    <Swiper slidesPerView="auto" spaceBetween={withLayout ? 40 : 0}>
      {posts.map((post, i) => (
        <SwiperSlide key={i} style={{ width: 'auto' }} className="!h-auto">
          <div
            className={cx('h-full max-w-[304px]', {
          ...
          >
            <Link href={`/blog/${post.href}`} className="group">
              <div className="mb-6 overflow-hidden rounded-3xl">
                <Image
                  src={post.image}
                  alt=""
                  className="h-[264px] rounded-2xl object-cover transition-transform duration-500 group-hover:scale-105"
                />
              </div>
